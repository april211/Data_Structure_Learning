// FAC：经过修改最后  AC 的版本
// 错误原因：各个结点的邻接表中可能有重复的结点，所以入队的都要先访问，并且置状态为 true
#include <stdio.h>
...
void BFS(LGraph Graph, Vertex S, void (*Visit)(Vertex))
{
    ...
    // 访问起始节点
    //（该处未将S入队列，而是单独处理了，可能造成了错误）
    (*Visit)(S);
    Visited[S] = true;

    // 将与 S 邻接的结点入队列
    for (PtrToAdjVNode ipt = (mg->G)[S].FirstEdge; ipt != NULL; ipt = ipt->Next)
    {
        Vertex v_id = ipt->AdjV; // 获取邻接点的下标
        if (!Visited[v_id])      // 如果未访问过该节点，则访问该节点
        {
            (*Visit)(v_id); // 访问并标记该节点
            Visited[v_id] = true;
